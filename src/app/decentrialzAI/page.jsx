  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await axios.get(
        `${backendUrl}/ai?query=${encodeURIComponent(trimmedMessage)}`,
        {
      ...
      const aiText =
        response.data?.trim() || "Sorry, I didn't understand that.";
  ...
  return (
    <div className="min-h-screen bg-[#Fff] flex flex-col justify-between relative">
      ...
      <div
        ...
        className="inner_chat_body flex flex-col items-center p-4 sm:p-6"
      >
        {messages.map((msg, index) => (
          ...
          >
            <div className="intercom-comment intercom-18ciyg5 ekd1qb41">
              <div
                className={`intercom-avtrru e1jqii293 ${
                  msg.type === "user" ? "justify-end" : ""
                }`}
              >
                <div
                  className={`intercom-hiupyl e1jqii290 ${
                    msg.type === "user" ? "justify-end" : ""
                  }`}
                >
                  <div
                    size="20"
                    shape="squircle"
                    className="intercom-3xwhyz e2rn66r0"
                  >
                    <img
                      src={
                        msg.type === "user"
              ...
              <div
                tabIndex="-1"
                className={`intercom-18ztwyf er4a1r20 ${
                  msg.type === "user" ? "flex justify-end " : ""
                }`}
              >
                <div
                  className={`intercom-block-paragraph e16pl8n50 intercom-ks2cl1 max-w-[75%] rounded-lg p-2 text-black`}
                >
        ...
        <div className=" fixing_bottom flex flex-col items-center justify-center py-6">
          <div className="flex w-[90%] max-w-[900px] flex-col items-center space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
            <input
              ...
              className="h-[60px] w-full flex-1 rounded-full border border-gray-200 px-6 text-sm focus:outline-none focus:ring-2 focus:ring-[#6A5AE0] sm:w-auto"
            />
            <SpeachRecognition
              onTranscriptUpdate={setUserMessage}
            />
            <button
              onClick={handleSendMessage}
              className="h-[54px] w-full rounded-full bg-gradient-to-r from-[#4A25E1] to-[#7B5AFF] font-semibold text-white shadow-[0px_21px_27px_-10px_rgba(96,60,255,0.48)] transition hover:opacity-90 sm:w-[192px]"
            >
              Submit
            </button>
          </div>

          <div className="easy-option-prompts flex justify-start px-11">
            <span className="prompt_option flex w-[auto] items-center gap-2">
              {" "}
              <svg
